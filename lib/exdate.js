    ...
    xformat: function(date, i18n) {
        ...
        (function() {
            var cn = {
                ...
                today: '今天 ',
                todayFormat: 'H:m',
                thisYearFormat: 'M-d H:m',
                olderYearFormat: 'yyyy-M-d H:m'
            };
            for(var key in cn) {
                if(!i18n[key]) {
                    i18n[key] = cn[key];
            ...
            if(minute <= 60) {
                ...
                if(r.compare(thisDate, date) == -1) { // 比今天零点早，即今天之前
                    ...
                    if(r.compare(thisYear, date) == -1) { // 比今年1月1号零点早，即今年之前
                        result = r.format(date, i18n.olderYearFormat);
                    } else {
                        result = r.format(date, i18n.thisYearFormat);
                    }
                } else {
                    result = i18n.today + r.format(date, i18n.todayFormat);
                }
            }
        }
        return result;
